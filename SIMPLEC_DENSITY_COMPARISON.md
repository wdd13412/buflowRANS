...
- 更可疑的是当前 `simplec_apply_low_mach_bounds` 每步把全场压力线性缩放到 `SIMPLE_MAX_PRESSURE_RANGE=200 Pa`；这会让总体压差满足目标，但可能压缩真实壁面压力结构并改变云图视觉重点。后续要提升物理精度，应把这个全场缩放替换为压力修正限幅/局部 bounded correction，并用本诊断确认壁面压力极值是否自然出现。
